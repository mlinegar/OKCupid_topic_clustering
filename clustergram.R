# this code is not written by me - I took it from here:
# https://www.r-bloggers.com/clustergram-visualization-and-diagnostics-for-cluster-analysis-r-code/
# I've just made two small changes - explicitly setting iter.max and nstart
# and changing the default algorithm used to algorithm="MacQueen"
clustergram.kmeans <- function(Data, k, iter.max=10, nstart=1, ...)
{
  ...
  cl <- kmeans(Data, k, iter.max = iter.max, nstart = nstart, algorithm="MacQueen", ...)
...
clustergram <- function(Data, k.range = 2:10 , 
                        ...
                        line.width = .004, add.center.points = T, iter.max=10, nstart=1)
{
  ...
  for(k in k.range)
  {
    k.clusters <- clustering.function(Data, k, iter.max, nstart)
